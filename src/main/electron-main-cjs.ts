...
function topY(winHeight: number) {
  // Posicionar no topo da tela com 20px de distância
  return 20 // 20px do topo
}
...
// IPC Handlers
ipcMain.handle('overlay:set-ignore', async (event: any, ignore: boolean) => {
  console.log('🔧 Main: set-ignore chamado:', ignore)
  return { success: true }
})

ipcMain.handle('overlay:force-interactive', async (event: any) => {
  console.log('🔧 Main: force-interactive chamado')
  return { success: true }
})

ipcMain.handle('overlay:open-external', async (event: any, url: string) => {
...
ipcMain.handle('overlay:close-app', async (event: any) => {
  console.log('🔧 Main: Fechando aplicação')
  app.quit()
  return { success: true }
})

ipcMain.handle('recorder:save', async (event: any, data: any) => {
  try {
...
function generateSRT(transcript: any[]) {
